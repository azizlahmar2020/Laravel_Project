<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="mt-6 space-y-4">
        ...
        <div class="flex items-center justify-end mt-4">
            ...
            <!-- Button for register -->
            <a href="{{ route('register') }}" class="ml-3 inline-flex items-center px-4 py-2 bg-green-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-600 focus:bg-green-600 active:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150">
                {{ __('Register') }}
            </a>

            <x-primary-button class="ml-3 bg-green-700 hover:bg-green-600 focus:bg-green-600 active:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
                {{ __('Log in') }}
            </x-primary-button>
        </div>
    </form>
